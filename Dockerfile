...
#RUN apt-get update && apt-get -y upgrade && apt-get install -y build-essential vim git wget ncbi-blast+ python python3 \
#	python-dev python-pip python-numpy python-scipy python-biopython python3-pip
...
RUN git clone --recursive https://gitlab.com/ezlab/busco.git && \
	cd busco && python3 setup.py install && cd .. && \
